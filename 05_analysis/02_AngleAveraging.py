...
def MakeSignalFigure():
    fig = PLT.figure(figsize = (16.5/2.54, 10/2.54), dpi=300)
    fig.subplots_adjust( \
                              top    = 0.82 \
                            , right  = 0.98 \
                            , bottom = 0.09 \
...
if __name__ == "__main__":
    ...
    fig.tight_layout()
    fig.savefig('../figures/zebra_wrist_raw.pdf', transparent = False, dpi = 300)
    PLT.show()
    ...
    fig.tight_layout()
    fig.savefig('../figures/zebra_wrist_aligned.pdf', transparent = False, dpi = 300)
    PLT.show()
